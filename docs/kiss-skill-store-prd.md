...
KISS 的产品定位是：一个面向 coding agent 的显式 skill store 和 context provider。用户必须先通过 `kiss add ...` 显式安装 skill；之后再通过 `/kiss <skill_name> ...`、`$kiss <skill_name> ...` 或 `kiss run <skill_name> ...` 指定要用的 skill。`run` 在默认 `markdown` runner 下只加载已安装且被点名的 skill，并把最小上下文输出给调用方 agent；后续推理、工具调用和文件编辑由外层 agent 完成。`run` 不自动下载或安装，也不调用任何大模型。
...
- 不提供 npm / pnpm / bunx shim、Linux 发行版包、Windows 包管理器入口、MSI 或 web registry UI。
...
6. **渐进增强**：先保证本地 CLI 跑通，再做远程下载、registry 和 adapter。
...
- KISS bridge skill/adapter 的职责是把用户输入 `/kiss <skill> [args...]` 转成 `kiss run <skill> [args...]`，再把 stdout 作为当前轮上下文交给 agent。
...
- 支持可选签名 registry entry，用于审计 registry 中 name/source/sha256 的绑定关系。
- 支持本地 registry trust policy，可要求 registry entry 必须签名并限制可信 public key。

### Phase 3：Adapter 与 context handoff

- `/kiss`、`$kiss` adapter 和 KISS bridge skill。
