...
KISS 是一个独立的 skill store 和 launcher。它借鉴 Vercel Skills 生态中“skill 是可分发目录、通过 CLI 安装、入口是 `SKILL.md`”的简单模型，但刻意不把 skill 安装到任何 agent 的原生扫描目录。
...
6. command runner 默认禁用；启用时必须经过 manifest 和 policy 检查。
...
KISS CLI
  ...
  |-- Runner: markdown prompt and optional scripts
  v
Agent receives only selected skill context
```

KISS 不负责替代 coding agent，也不负责把 skill 同步到 Claude、Codex、Cursor、Gemini 等工具的全局目录。安装和运行是两个显式动作：`kiss add` 负责保存 skill 与元数据，`kiss run` 只负责加载已安装且被点名的 skill。
...
KISS 安装 skill 后，必须在 `$KISS_HOME/skills.db.json` 保存元数据。MVP 使用 JSON 文件作为核心数据库，而不是 SQLite、DuckDB 或 Parquet，理由是：可读、可审计、易删除、方便测试、无额外 native 依赖；后续如果需要并发写入或复杂查询，再优先评估 SQLite。DuckDB + Parquet 可以作为可选分析/导出层，但不进入 `run/list/show` 的核心路径。
...
- `schema_version`：数据库结构版本，用于未来迁移。
- `skills`：以 KISS 本地 skill 名称为 key 的对象。
- `name`：KISS 本地短名，也是 `$KISS_HOME/skills/<name>` 的目录名；用户用它执行 `kiss run <name>`。
- `full_name`：来源限定全称，用于审计、去重、迁移和未来 update。格式建议为 `github:owner/repo/path/to/skill`、`https://example.com/skill.tar.gz` 或 `local:/abs/path/to/skill`。
...
未来可以提供可选导出或分析命令，例如 `kiss export metadata --format parquet`，但 `kiss run`、`kiss list` 和 `kiss show` 不应依赖 DuckDB。
...
解析只发生在显式安装或更新命令中，`kiss run` 不触发 resolver。未来解析顺序：
...
MVP 不需要中心化服务；可以直接使用 GitHub raw、Git tag、压缩包 URL 或本地路径。后续再增加签名 registry。

## 7. Run 协议

`kiss run <skill> <args>` 只运行已安装 skill；如果 skill 不在本地数据库或安装目录中，返回错误并提示先执行 `kiss add ...`，不得自动下载。它的输出应当适合 agent 消费，而不是把所有已安装 skill 都暴露给 agent。
...
- If the skill references scripts, run only the declared script entrypoints.
```

如果 `runner.type = "command"`，则执行 manifest 声明的命令，并把 stdout/stderr 返回给 agent；默认仍然先展示计划，除非用户传入 `--yes`。
...
| Runner | 输出上下文或执行脚本 | `markdown` runner 优先 |
| Adapter | `/kiss`、`$kiss` 接入 | 薄封装，调用 CLI |

## 11. 实现技术路线

KISS 的实现路线应先把“可运行的本地 CLI”做扎实，再增加 registry、签名和更多 agent adapter。核心判断是：**不要先做平台，不要先做 UI，不要先做复杂生态；先做一个能被 `/kiss` 和 `$kiss` 稳定调用的 `kiss run`**。
...
| MVP 分发 | 预编译单文件 CLI | 不需要 | 通过 install script、Homebrew、GitHub Release 或系统包管理器安装。 |
| 兼容入口 | npm / pnpm / bunx shim | 选择该入口的用户需要 | 只作为补充入口，可下载并转发到预编译 binary。 |
...
- `kiss update` 下载到 side-by-side 目录，展示 manifest 和 `SKILL.md` 摘要 diff，确认后再替换。
...
- adapter 不实现 installer/loader 逻辑，只调用 CLI，避免多份实现行为不一致。

#### M5：command runner 与权限策略
...
- command 必须是 manifest 中声明的相对路径。
- 禁止 symlink 跳出 skill 目录。
- 默认 cwd 是临时工作目录或当前 workspace，由用户显式选择。
- 默认不把 secret 环境变量传入 command runner；需要 allowlist。
...
- `manifest`：合法 TOML、缺省 manifest、非法 runner、非法权限。
